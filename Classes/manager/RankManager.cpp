...
void RankManager::createDatabase()
{
	...
	std::string sqlStr;
	sqlStr = "create table IF NOT EXISTS RankTable(NAME nvarchar(32),STACK integer,SCORE integer)";
...
void RankManager::insertData(char* name, int stack, int score) {
	...
	std::string sqlStr;
		...
		sqlStr = CCString::createWithFormat(	//데이터 삽입을 위한 쿼리문 생성
				"insert into RankTable(name,stack,score) values('%s',%d,%d)",name,stack,score)->getCString();
...
void RankManager::deleteData(int minimum)
{
	...
	result  = sqlite3_open(dbfileName.c_str(),&pDB);	 //db 열기
		...
		std::string sqlStr;
				sqlStr = sqlStr = CCString::createWithFormat("delete from RankTable where score<=%d",minimum)->getCString();
				result = sqlite3_exec(pDB,sqlStr.c_str(),NULL,NULL,&errMsg);
				if(result!=SQLITE_OK)
							{
								CCLog("Open Error : Code:%d Msg:%s",result,errMsg);
